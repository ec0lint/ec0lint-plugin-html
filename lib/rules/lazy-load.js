    ...
    return {
      Tag(node) {
        ...
        if (
          ...
          const co2Reduction = fileName
            ? calculateCO2LazyLoadReduction(fileName).toFixed(2)
            : null;
          const message =
            `Lazy loading can be used for ${
              fileName ? fileName + "." : "this file."
            }` +
            `${
              co2Reduction !== null
                ? " Estimated CO2 reduction that you can achieve is: " +
                  co2Reduction +
                  "g"
                : ""
            }`;
